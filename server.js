...
app.use('/api/submit-construction-worker', limiter); // Add rate limiting for construction worker
...
const validateConstructionWorkerRequest = [
    body('firstName').trim().notEmpty().withMessage('First name is required')
...
// Construction Worker Application Endpoint
app.post('/api/submit-construction-worker', validateConstructionWorkerRequest, async (req, res, next) => {
    try {
        ...
        const adminMailOptions = {
            ...
            subject: 'New Construction Worker Application',
            replyTo: email,
            html: `
        ...
        const workerMailOptions = {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    ...
                    <p>Best regards,<br>Construction Armor Team</p>
                </div>
            `
        };
        ...
        res.status(200).json({ 
            message: 'Construction worker application submitted successfully',
            status: 'success'
        });
...
// Updated Company Request Endpoint (for construction companies)
app.post('/api/submit-company', validateCompanyRequest, async (req, res, next) => {
    try {
        ...
        const adminMailOptions = {
            from: process.env.EMAIL_USER,
            to: process.env.ADMIN_EMAIL || process.env.EMAIL_USER,
            subject: 'New Construction Company Request',
            replyTo: email,
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        logger.info('Construction company request submitted successfully', {
            company: companyName,
...
app.get('/api/health', (req, res) => {
    res.status(200).json({ 
        ...
        timestamp: new Date().toISOString(),
        endpoints: [
            '/api/submit-construction-worker',
            '/api/submit-company',
        ]
    });
});
...
const server = app.listen(PORT, () => {
    ...
    logger.info('  POST /api/submit-construction-worker');
